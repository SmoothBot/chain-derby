      ...
      <div ref={shareableContentRef} className="p-6 bg-card rounded-lg"
        ...
        <div className={`flex ${isMobile ? 'flex-col gap-4' : 'justify-between items-center'}`}>
          ...
          <div className={`flex items-center gap-2 ${isMobile ? 'self-start' : ''}`}>
            <Badge variant="outline" className={`${isMobile ? 'px-2 py-1 text-sm' : 'px-3 py-1.5 text-lg'}`}>
            ...
            >
              ...
              <span className={isMobile ? 'text-xs' : ''}>{isCapturing ? "Capturing..." : "Share Results"}</span>
            </Button>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent>
        ...
        {sortedResults.length > 0 && (
          <div className="mb-8 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800/40">
            {isMobile ? (
              /* Mobile winner layout */
              <div className="space-y-4">
                {/* Header with logo and rank */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div className="h-12 w-12 rounded-full bg-gradient-to-br from-yellow-300 to-yellow-500 flex items-center justify-center">
                        <Image 
                      ...
                      <div className="absolute -bottom-1 -right-1 bg-white dark:bg-gray-900 rounded-full p-0.5 shadow-md">
                        <Trophy className="h-4 w-4 text-yellow-500" />
                      </div>
                    </div>
                    <div>
                      <h3 className="text-lg font-bold">{sortedResults[0].name} Wins!</h3>
                      <p className="text-xs text-muted-foreground">Average: <span className="font-bold">{sortedResults[0].averageLatency}ms</span></p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-yellow-500">#1</p>
                    {sortedResults[0].totalLatency && (
                      <p className="text-xs text-muted-foreground">Total: {(sortedResults[0].totalLatency / 1000).toFixed(2)}s</p>
                    )}
                  </div>
                </div>
        ...
        {/* Race statistics */}
        <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-1 sm:grid-cols-3 gap-4'} mb-8`}>
          <div className={`bg-blue-50 dark:bg-blue-950/30 rounded-lg ${isMobile ? 'p-3' : 'p-4'} flex items-center gap-3`}>
            <div className={`${isMobile ? 'h-8 w-8' : 'h-10 w-10'} rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center`}>
              <Zap className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-blue-600 dark:text-blue-400`} />
            </div>
            <div>
              <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Fastest Time</p>
              <p className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold`}>{fastestTime}ms</p>
              <p className="text-xs text-muted-foreground">{fastestChain}</p>
            </div>
          </div>
          
          <div className={`bg-purple-50 dark:bg-purple-950/30 rounded-lg ${isMobile ? 'p-3' : 'p-4'} flex items-center gap-3`}>
            <div className={`${isMobile ? 'h-8 w-8' : 'h-10 w-10'} rounded-full bg-purple-100 dark:bg-purple-900/50 flex items-center justify-center`}>
              <BarChart3 className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-purple-600 dark:text-purple-400`} />
            </div>
            <div>
              <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Average Time</p>
              <p className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold`}>{avgTime}ms</p>
              <p className="text-xs text-muted-foreground">Across all chains</p>
            </div>
          </div>
          
          <div className={`bg-green-50 dark:bg-green-950/30 rounded-lg ${isMobile ? 'p-3' : 'p-4'} flex items-center gap-3`}>
            <div className={`${isMobile ? 'h-8 w-8' : 'h-10 w-10'} rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center`}>
              <Medal className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-green-600 dark:text-green-400`} />
            </div>
            <div>
              <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Successful Chains</p>
              <p className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold`}>{totalSuccessful}/{results.length}</p>
        ...
        {isMobile ? (
          ...
          <ul className="space-y-2">
            {sortedResults.map((result, index) => (
              <li key={result.chainId} className={`p-3 rounded-lg border ${
                index === 0 ? "bg-yellow-50 dark:bg-yellow-950/20 border-yellow-200 dark:border-yellow-800" : 
                index === 1 ? "bg-gray-50 dark:bg-gray-800/20 border-gray-200 dark:border-gray-700" : 
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center">
                      ...
                      {index > 2 && <span className="text-sm font-medium mr-1">#{index + 1}</span>}
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{result.txCompleted}/{result.txTotal} tx</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-mono font-medium text-sm">{result.averageLatency}ms</p>
                    {result.totalLatency && (
                      <p className="text-xs text-muted-foreground">{(result.totalLatency / 1000).toFixed(2)}s total</p>
                    )}
                  </div>
                </div>
              </li>
            ...
            <div className="space-y-4">
              {sortedResults.map(result => (
                <div key={`details-${result.chainId}`} className="border rounded-md p-4">
                  ...
                  <div className={`grid ${isMobile ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-5'} gap-2`}>
